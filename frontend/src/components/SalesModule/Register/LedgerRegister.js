...
console.log(endOfWeek1String);
    ...
    useEffect(() => {  
        ...
        if(dFilter === "all"){
            ...
            setPendingRows(null)
            axios
            .get(employeeData.URL + '/api/v1/salesOrder/get-ledger',{
                withCredentials: true
            })
            .then((response) => {
                ...
                setAllRows(rows => {
                    let newRows = response.data.data.ledger.rows.map((row,index) => {return(
                        {
                            "SNO": index + 1,
                            ...row
                        }
        ...
        <>            
            <Row className = {classes['Row']}>
               {/* // {console.log(classes['Title'])} */}
                <Col md = {14}><p className = {classes['Title']}>Ledger</p></Col>
