...
console.log(endOfWeek1String);
                ...
                else if(dFilter=="yesterday")
                {
                    setPendingRows(rows => {
                        ...
                        let newRows = response.data.data.dat1.filter((row,index) => {return(
                           row.invoice_date == yesterdayDate
                        );})
                        return newRows;
                    })  
                }
                else if (dFilter == "finYear")
                {
                    setPendingRows(rows => {
                        ...
                        let newRows = response.data.data.dat1.filter((row,index) => {return(
                           row.invoice_date.split("-")[2] == currentYear
                        );})
                        return newRows;
                    })  
                }
                else if (dFilter == "week")
                {
                    setPendingRows(rows => {
                        ...
                        let newRows = response.data.data.dat1.filter((row,index) => {return(
                           row.invoice_date>=beginOfWeek1String && row.invoice_date<=endOfWeek1String
                        );})
                        return newRows;
                    })
                }
            ...
            setPendingRows(null)
            axios
            .get(employeeData.URL + '/api/v1/salesInvoice/get-all-invoice-by-week',{
                withCredentials: true
            })
            .then((response) => {
                console.log(response);
                ...
                if(fromDate !== "" && toDate !== ""){
                  ...
                  console.log(to)
                    axios
                    .get(employeeData.URL + '/api/v1/salesInvoice/get-all-invoice/?from=' + from + "&to=" + to,{
                        withCredentials: true
                    })
                    .then((response) => {
        ...
        <>            
            <Row className = {classes['Row']}>
               {/* // {console.log(classes['Title'])} */}
                <Col md = {14}><p className = {classes['Title']}>Sales Invoice Register</p></Col>
