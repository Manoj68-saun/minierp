...
const VoucherDetails = (props) => {
    ...
    useEffect(() => {
        ...
        axios
            .get(employeeData.URL + '/api/v1/voucher',{
                    ...
                    let newRows = response.data.data.order.rows.map((row,index) => {return(
                        {
                            "D": <Tooltip placement="bottom" title = "Delete" color = "red">
                                    <Link  to = "#" style = {{color: "red", fontWeight: "bolder"}} >
                                        <AiFillDelete  onClick = {(event) => deleteHandler(event,index,response.data.data.order.rows,row['voucher_code'])} style = {{color: "red", fontWeight: "bolder", fontSize : "0.8rem"}}/>
                                    </Link>
                                </Tooltip>,
                            "V": <Tooltip placement="bottom" title = "View" color = "#1777C4">
                                <Link  to = { "/sales/transaction/voucher-form-view/" + row['voucher_code']} style = {{color: "#1777C4", fontWeight: "bolder"}} >
    ...
    const deleteHandler = (event,index,newrows,code) => {
        ...
        setRows(null)
        axios
            .delete(employeeData.URL + '/api/v1/voucher/' + code.toString(),{
                withCredentials: true
            })
            .then((response) => {
                console.log(response);
                    const rowvals = newrows.filter(row => row['voucher_code'] !== code)
                    setRows( rows => {
                        let newDRows = rowvals.map((row,index) => {return(
                            {
                                "D": <Tooltip placement="bottom" title = "Delete" color = "red">
                                        <Link  to = "#" style = {{color: "red", fontWeight: "bolder"}} >
                                            <AiFillDelete  onClick = {(event) => deleteHandler(event,index,rowvals,row['voucher_code'])} style = {{color: "red", fontWeight: "bolder", fontSize : "0.8rem"}}/>
                                        </Link>
                                    </Tooltip>,
                                "V": <Tooltip placement="bottom" title = "View" color = "#1777C4">
                                    <Link  to = {"/sales/transaction/voucher-form-view/" + row['voucher_code']} style = {{color: "#1777C4", fontWeight: "bolder"}} >
                    ...
                    let newDRows = newrows.map((row,index) => {return(
                        {
                            "D": <Tooltip placement="bottom" title = "Delete" color = "red">
                                    <Link  to = "#" style = {{color: "red", fontWeight: "bolder"}} >
                                        <AiFillDelete  onClick = {(event) => deleteHandler(event,index,newrows,row['voucher_code'])} style = {{color: "red", fontWeight: "bolder", fontSize : "0.8rem"}}/>
                                    </Link>
                                </Tooltip>,
                            "V": <Tooltip placement="bottom" title = "View" color = "#1777C4">
                                <Link  to = { "/sales/transaction/voucher-form-view/" + row['voucher_code']} style = {{color: "#1777C4", fontWeight: "bolder"}} >
    ...
    return(
        <>            
            <Row className = {classes['Row']}>
                <Col md = {14}><p className = {classes['Title']}>Voucher</p></Col>
            ...
            <Row className = {classes['Row']}>
                ...
                <Col className = {classes['Col']} lg = {5} md = {7}>
                    <Link to = "/sales/transaction/Voucher-create"><button className = {classes['ProfileButton']}>Add an Voucher</button></Link>
                </Col>
            </Row>
            <p></p>
            {
