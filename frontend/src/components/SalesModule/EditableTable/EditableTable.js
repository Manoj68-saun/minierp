...
const TableRenderer = (props) => {
    ...
    const [defaultColumnWidths] = useState(props.columns.map(col => 
            {
                if(col.name === "D" || col.name === "E" || col.name === "V")
                    ...
                    return({columnName: col.name, width: 180});
            }
        ))
    ...
    const LeaveFormatter = ({ value }) => {
        ...
        const findIndex = props.leaveTypes.rows.findIndex(element => element.LEAVETYPE_CODE === value)
        return(
            <b className = {classes['Cell']}>
                {findIndex === -1 ? null : props.leaveTypes.rows[findIndex]['LEAVETYPE_DESC']}
            </b>
        );
    }
    ...
    const LeaveTypeProvider = props => (
        <DataTypeProvider
            formatterComponent={LeaveFormatter}
            {...props}
        />
    );
    ...
    const PeriodFormatter = ({ value }) => {
        ...
        const findIndex = props.periodList.rows.findIndex(element => element.KEY === value)
        return(
            <b className = {classes['Cell']}>
            ...
            <StylesProvider injectFirst>  
                <div className = {classes['TableView']} id = "styledTable">
                    ...
                    <Paper square elevation = {0} className = {classes['Paper']}>
                        <Scrollbars autoHeight autoHeightMax={ props.val ? ( window.innerHeight*0.75 ) : ( window.innerHeight*0.65 )}>
                            <Grid
                                rows={rows}
                                columns={columns} 
                            ...
                            >   
                                ...
                                <Table columnExtensions={tableColumnExtensions} tableComponent={TableComponent} headComponent = {HeadComponent} containerComponent = {ContainerComponent}/>
