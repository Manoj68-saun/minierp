...
const TaxDetails = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(employeeData.URL + "/api/v1/tax", {
      ...
      .then((response) => {
        ...
        setColumns((columns) => {
          let newCols = response.data.data.tax.fields.map((col) => {
            return {
              name: col.name,
        ...
        setRows((rows) => {
          let newRows = response.data.data.tax.rows.map((row, index) => {
            return {
              D: (
              ...
              V: (
                <Tooltip placement="bottom" title="View" color="#1777C4">
                  <Link
                    to={"/sales/norms/tax-form-view/" + row["tax_code"]}
  ...
  const deleteHandler = (event, index, newrows, code) => {
    ...
    setRows(null);
    axios
      .delete(employeeData.URL + "/api/v1/tax/" + code.toString(), {
        withCredentials: true,
      })
      .then((response) => {
        console.log(response);
        const rowvals = newrows.filter((row) => row["tax_code"] !== code);
        setRows((rows) => {
              ...
              D: (
                <Tooltip placement="bottom" title="Delete" color="red">
                  <Link to="#" style={{ color: "red", fontWeight: "bolder" }}>
                    <AiFillDelete
                      onClick={(event) =>
                        deleteHandler(event, index, rowvals, row["tax_code"])
                      }
                      style={{
                        color: "red",
                        fontWeight: "bolder",
              ...
              D: (
                <Tooltip placement="bottom" title="Delete" color="red">
                  <Link to="#" style={{ color: "red", fontWeight: "bolder" }}>
                    <AiFillDelete
                      onClick={(event) =>
                        deleteHandler(event, index, newrows, row["tax_code"])
                      }
                      style={{
                        color: "red",
                        fontWeight: "bolder",
    ...
    <>
      ...
      <Row className={classes["Row"]}>
        ...
        <Col className={classes["Col"]} lg={5} md={7}>
          <Link to="/sales/norms/tax-create">
            <button className={classes["ProfileButton"]}>Add an Tax</button>
          </Link>
        </Col>
      </Row>
      <p></p>
