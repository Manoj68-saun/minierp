...
const Consumption = (props) => {
  ...
  // Function to update total quantity when individual quantity changes
  // const handleQuantityChange = (index, value) => {
  //   props.setData((data) => {
  //     const newQuantity = parseFloat(value) || 0;
  //     const newData = { ...data };
  //     newData.reqDetail[index].qty = newQuantity; // Update individual quantity
  //     // Recalculate total quantity
  //     const newTotalQuantity = newData.reqDetail.reduce(
  //       (total, item) => total + (item.qty || 0),
  //       0
  //     );
  //     setTotalQuantity(newTotalQuantity); // Update total quantity state
  //     return newData;
  //   });
  // };
  ...
  const handlechangee1 = async (changedValues, allValues, i) => {
    ...
    setSizef(sizef);
    const values = form.getFieldsValue(["prodConsumDet"]);
    ...
    // }
    values.prodConsumDet[i].Mode = false;
    values.prodConsumDet[i].size_code = null;
    values.prodConsumDet[i].uom_code = null;
    values.prodConsumDet[i].quality_code = null;
      ...
      axios
        .get(
          employeeData.URL +
            ...
            data.prodConsumDet[i].item_code,
          {
        ...
        .then((response) => {
          ...
          if (uomOptions.length === 1) {
            values.prodConsumDet[i].uom_code = uomOptions[0].value;
            form.setFieldsValue(values);
          }
  ...
  const handleValueChanges = (changedValues, allValues) => {
    props.setData((data) => {
      return {
        ...data,
        prodConsumDet: allValues.prodConsumDet,
  ...
  return props.editMode ? (
    ...
    >
      <div className="component-container">
        <Form.List name="prodConsumDet" initialValue={props.data}>
          {(fields, { add, remove }) => {
            return (
              <>
                    ...
                    >
                      ...
                      >
                        ...
                        <td style={{ border: "1px solid white" }}>
                          <Form.Item
                            {...restField}
                            name={[name, "no_of_picces"]}
                            fieldKey={[fieldKey, "no_of_picces"]}
                            // label={<div style={{ padding: "0rem 0.5rem", fontSize: "0.6rem", fontWeight: "bold" }} className={classes['Label']}></div>}
                            // rules={[
                            //   {
                            //     required: true,
