...
const OpeningDetails = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(employeeData.URL + "/api/v1/opening", {
      ...
      .then((response) => {
        ...
        setColumns((columns) => {
          let newCols = response.data.data.Openning.fields.map((col) => {
            return {
              name: col.name,
        ...
        setRows((rows) => {
          let newRows = response.data.data.Openning.rows.map((row, index) => {
            return {
              D: (
                    ...
                    <AiFillDelete
                      onClick={(event) =>
                        deleteHandler(
                          event,
                          index,
                          response.data.data.Openning.rows,
                          row["ssh_code"]
                        )
                      }
                      style={{
              ...
              V: (
                <Tooltip placement="bottom" title="View" color="#1777C4">
                  <Link
                    to={
                      "/stock/transaction/opening-balance-view/" +
                      row["ssh_code"]
                    }
  ...
  const deleteHandler = (event, index, newrows, code) => {
    ...
    setRows(null);
    axios
      .delete(employeeData.URL + "/api/v1/opening/" + code.toString(), {
        withCredentials: true,
      })
      .then((response) => {
        console.log(response);
  ...
  return (
    <>
      <Row className={classes["Row"]}>
        <Col md={14}>
          <p className={classes["Title"]}>Opening Details</p>
        </Col>
      ...
      <Row className={classes["Row"]}>
        ...
        <Col className={classes["Col"]} lg={5} md={7}>
          <Link to="/stock/transaction/opening-balance-create">
            <button className={classes["ProfileButton"]}>Add Opening</button>
          </Link>
        </Col>
      </Row>
      <p></p>
