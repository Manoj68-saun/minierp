...
const RequisitionView = (props) => {
  ...
  useEffect(() => {
    ...
    const fetchAllData = async () => {
      if (!itemm || !itemm.stockDetail) {
        console.error("itemm or itemm.stockDetail is null or undefined", itemm);
        return;
      }

      try {
        await Promise.all(
          itemm.stockDetail.map((item, index) => fetchData(item, index))
        );
      } catch (error) {
  ...
  const handleSave = (event) => {
    ...
    postData.stockDetail = postData.stockDetail.filter(
      (detail) => detail !== null && detail !== undefined
    );

    // Process the filtered array
    postData.stockDetail.forEach((lang, index) => {
      if (!lang.uniq_id) {
        postData.stockDetail[index] = {
          ...lang,
          PARAM: "INSERT",
        };
      } else {
    ...
    storeData.stockDetail.forEach((lang, index) => {
      const dataIndex = postData.stockDetail.findIndex(
        (element) => element.uniq_id === lang.uniq_id
      );
      if (dataIndex === -1) {
        postData.stockDetail.push({
