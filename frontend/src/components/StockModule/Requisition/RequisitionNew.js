...
const RequisitionNew = (props) => {
  ...
  const [allData, setAllData] = useState({
    reqHdr: [
      {
        req_date: dayjs(new Date()).format("DD-MM-YYYY"),
        req_time: dayjs(new Date()).format("HH:mm:ss"),
        dept_cd: null,
        emp_cd: null,
      },
    ],
  ...
  const handleSave = (event) => {
    ...
    if (formv) {
      ...
      if (
        allData.reqHdr[0].req_date == null ||
        allData.reqHdr[0].req_time == null ||
        allData.reqHdr[0].dept_cd == null ||
        allData.reqHdr[0].emp_cd == null
      ) {
        message.error({
      ...
      const allFieldsNull = allData.reqDetail.every(
        (detail) =>
          detail.item_code === null &&
          detail.uom_code === null &&
          detail.size_code === null &&
      ...
      axios
        .post(employeeData.URL + "/api/v1/requisition", postData, {
          withCredentials: true,
          credentials: "include",
        })
