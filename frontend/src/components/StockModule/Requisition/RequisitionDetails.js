...
const RequisitionDetails = (props) => {
  ...
  useEffect(() => {
    ...
    axios
      .get(employeeData.URL + "/api/v1/requisition", {
              ...
              return {
                ...
                V: (
                  <Tooltip placement="bottom" title="View" color="#1777C4">
                    <Link
                      to={
                        "/stock/transaction/requisition-view/" + row["req_code"]
                      }
  ...
  const deleteHandler = (event, index, newrows, code) => {
    ...
    setRows(null);
    axios
      .delete(employeeData.URL + "/api/v1/requisition/" + code.toString(), {
        withCredentials: true,
      })
      .then((response) => {
        console.log(response);
        const rowvals = newrows.filter((row) => row["req_code"] !== code);
        setRows((rows) => {
              ...
              D: (
                <Tooltip placement="bottom" title="Delete" color="red">
                  <Link to="#" style={{ color: "red", fontWeight: "bolder" }}>
                    <AiFillDelete
                      onClick={(event) =>
                        deleteHandler(event, index, rowvals, row["req_code"])
                      }
                      style={{
                        color: "red",
                        fontWeight: "bolder",
          ...
          let newDRows = newrows.map((row, index) => {
            return {
              D: (
                <Tooltip placement="bottom" title="Delete" color="red">
                  <Link to="#" style={{ color: "red", fontWeight: "bolder" }}>
                    <AiFillDelete
                      onClick={(event) =>
                        deleteHandler(event, index, newrows, row["req_code"])
                      }
                      style={{
                        color: "red",
                        fontWeight: "bolder",
              ...
              V: (
                <Tooltip placement="bottom" title="View" color="#1777C4">
                  <Link
                    to={"/stock/transaction/requisition-view" + row["req_code"]}
                    style={{ color: "#1777C4", fontWeight: "bolder" }}
                  >
                    <EditOutlined
                      style={{
  ...
  return (
    <>
      <Row className={classes["Row"]}>
        <Col md={14}>
          <p className={classes["Title"]}>Requisition Details</p>
        </Col>
      ...
      <Row className={classes["Row"]}>
        ...
        <Col className={classes["Col"]} lg={5} md={7}>
          <Link to="/stock/transaction/requisition-create">
            <button
              style={{
