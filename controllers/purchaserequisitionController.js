...
exports.getAllRequisition = wrapper(async (req, res, next) => {
  ...
  const Requisition = await client.query(
    `select rq_code, timestamptostring(plan_date)  plan_date, timestamptostring(requirement_date)  requirement_date, get_department(dept_code) department, get_employee(requester_code) employee from pur_rm_requisition_hdr where marked is null`
  );
  res.status(200).json({
    status: "success",
    data: {
