...
const generateReqId = async (client) => {
  ...
  const response1 = await client.query(
    `SELECT MAX(Req_code)M FROM PUR_STORE_REQ_HDR`
  );
...
exports.getAllRequisition = wrapper(async (req, res, next) => {
  ...
  const Requisition = await client.query(
    `select req_code, timestamptostring(req_date) req_date, req_time,get_department(dept_cd)department,get_employee(emp_cd)employee from PUR_STORE_REQ_HDR where marked is null`
  );
  res.status(200).json({
    status: "success",
    data: {
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        //   });
        // }
        const query = `INSERT INTO ${arr[i].tableName} (Req_code,  ${fields}) VALUES ('${openingBalanceCode}',  ${values})`;
        console.log(query);
