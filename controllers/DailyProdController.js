...
exports.getAllRequisition = wrapper(async (req, res, next) => {
  ...
  const Requisition = await client.query(
    `select ssh_code, timestamptostring(stock_date)  stock_date, parse_time_string(from_time::VARCHAR) from_time, parse_time_string(to_time::VARCHAR) to_time from SL_STOCK_STATUS_HDR where marked is null`
  );
  res.status(200).json({
    status: "success",
    data: {
...
exports.getHsnForItem = wrapper(async (req, res, next) => {
  ...
  const size = await client.query(
    `select rate_per_unit from PUR_MST_METER  where meter_code=${req.params.code}`
  );
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        const query = `INSERT INTO ${arr[i].tableName} (ssh_code,  ${fields}) VALUES ('${dailyProdCode}',  ${values})`;
        console.log(query);
        ...
        for (let j = 0; j < arr1.length; j++) {
          ...
          fields = fields.slice(0, -2);
          values = values.slice(0, -2);
          const query = `INSERT INTO ${arr[i].tableName} (${arr[i].uniqueReqIdentifier}, ${fields}) VALUES ('${dailyProdCode}', ${values})`;
          console.log(query);
          await client.query(query);
        }
      }
