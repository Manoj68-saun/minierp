...
exports.getAllAcc = wrapper(async (req, res, next) => {
  ...
  const account = await client.query(
    `SELECT account_code, account_name, ACCOUNT_group_cd, add1, add2,city_code, pin_code, ph1, pan_no, email,
    bank_account_nO, ifsc_code, account_type,trans_type FROM view_account_master`
  );
  res.status(200).json({
    status: "success",
    data: {
...
exports.getAccData = wrapper(async (req, res, next) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    query += ` WHERE ${arr[i].uniqueAccIdentifier}='${req.params.code}'`;
    console.log(query);
  ...
  for (let i = 0; i < arr.length; i++) {
    if (req.body[arr[i].responseFieldName]) {
      if (!arr[i].typeArray) {
        ...
        const query = `INSERT INTO ${arr[i].tableName} (ACCOUNT_CODE, ACCOUNT_TYPE, ${fields}) VALUES ('${accountCode}', 'A', ${values})`;
        console.log(query);
